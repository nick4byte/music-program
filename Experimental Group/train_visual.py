...
from model_tasnet_refine import VisualGuidedConvTasNet, stft


def calculate_metrics(ref, est, mix):
    ref = ref.detach().cpu().numpy().squeeze(1)   # [B, L]
    est = est.detach().cpu().numpy().squeeze(1)   # [B, L]
    mix = mix.detach().cpu().numpy().squeeze(1)   # [B, L]
    delta = 1e-7
...
def mixture_consistency_loss(pred_wav, mix_wav, attn_weights, attn_reg_weight=0.01, eps=1e-8):
    pred_B = mix_wav - pred_wav
    recon  = pred_wav + pred_B
    l_mix  = F.l1_loss(recon, mix_wav)
    entropy = -(attn_weights * (attn_weights + eps).log()).sum(dim=-1).mean()
    l_attn  = attn_reg_weight * entropy
    return l_mix + l_attn


def compute_psl_gt_cpu(s_a_wav, mix_mag, n_fft=2048, hop=512, eps=1e-8):
    # s_a_wav, mix_mag 都先 detach 移到 CPU 計算，不佔 GPU 記憶體
    s_a_cpu  = s_a_wav.detach().cpu().float()
    mix_cpu  = mix_mag.detach().cpu().float()
    with torch.no_grad():
        mag_a, _ = stft(s_a_cpu, n_fft=n_fft, hop=hop)
        energy_a   = mag_a.mean(dim=1)
        energy_mix = mix_cpu.mean(dim=1)
        p_gt = torch.sigmoid(2 * energy_a - energy_mix)
    return p_gt  # 留在 CPU
...
def save_bad_case(epoch, step, mix, s_a, pred_s_a):
    ...
    torch.save({"mix": mix.cpu(), "s_a": s_a.cpu(), "pred_s_a": pred_s_a.cpu()}, path + ".pt")
...
CONFIG = {
    ...
    'checkpoint_dir':   '/content/drive/MyDrive/checkpoints_tasnet',
...
def train():
    ...
    model = VisualGuidedConvTasNet().to(CONFIG['device'])
    ...
    for epoch in range(start_epoch, CONFIG['num_epochs']):
        ...
        psl_weight = min(0.2, 0.2 * epoch / 50)
        ...
        for step, batch in enumerate(pbar):
            ...
            with autocast(device_type='cuda'):
                pred_wav, pred_mag, mask, mix_mag, mix_phase, p_hat, attn_weights = model(
                    mix, app_A, mot_A, app_B, mot_B
                )

            # PSL 在 CPU 上計算，不佔 GPU 記憶體
            p_gt     = compute_psl_gt_cpu(s_a, mix_mag).to(CONFIG['device'])
            p_hat_f  = p_hat.float()
            psl_loss = F.binary_cross_entropy_with_logits(p_hat_f, p_gt)

            total_loss = si_sdr_loss(pred_wav, s_a) * w.mean() + psl_weight * psl_loss
